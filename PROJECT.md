# 🍔 SABOR RAIZ — Definição Completa do Projeto
...
**SABOR RAIZ** é um aplicativo web de delivery de hambúrgueres artesanais com design "Premium Elevado". O app permite que clientes naveguem pelo cardápio, montem pedidos personalizados (removendo ingredientes), finalizem a compra e acompanhem o status em tempo real. Possui também um painel administrativo completo para gerenciar pedidos, cardápio, banners e clientes.
...
│
├── public/
│   └── logo.png                  # Logo SABOR RAIZ (~1.7MB)
│
├── supabase/
...
**Categorias pré-definidas**: Hambúrgueres, Combos, Bebidas, Sobremesas
...
### Carrinho (Zustand)
- Persistido em `localStorage` com chave `sabor-raiz-cart`
...
- Fallback: se query falhar, verifica se email === `sabor@admin.com`
...
### Fluxo do Admin
```
Login (sabor@admin.com) → Header menu → "Painel Admin"
...
- Admin é detectado por `(auth.jwt() ->> 'email') = 'sabor@admin.com'` na policy de profiles
...
### Repositório GitHub
- **Repo**: `thomasttda/sabor-raiz-ios`
...
- [ ] Admin promovido: `UPDATE profiles SET role = 'admin' WHERE email = 'sabor@admin.com'`
